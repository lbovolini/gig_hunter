...
class AcademicoController 
{
	...
	public function criar()
	{
		...
		if (!$vazioAcad && !$vazioEnd) {
			if (!Academico::getAcademico($cpf)) {	
				if ($email == $confirmacao_email) {
					if ($senha == $confirmacao_senha) {
						...
						echo "<script> alert('Senhas não conferem!'); location.href='/View/Academico/Create.php'; </script>";
					}
				}
				else {
					echo "<script> alert('Emails não conferem!'); location.href='/View/Academico/Create.php'; </script>";
				}
			}
			else {
			...
			echo "<script> alert('Todos os campos são de preenchimento obrigatório!'); location.href='/View/Academico/Create.php'; </script>";
		}
